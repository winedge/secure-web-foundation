...
serve(async (req) => {
  ...
  try {
    ...
    if (action === "publish_due") {
      ...
      return jsonResponse({ published: results.filter((r) => r.success).length, results });
    }

    if (action === "publish_now") {
      if (!post_id) throw new Error("post_id required");
      const { data: post, error } = await supabase.from("social_posts").select("*").eq("id", post_id).single();
    ...
    if (action === "fetch_engagement") {
      ...
      for (const post of publishedPosts) {
        ...
        if (metrics) {
          await supabase.from("social_posts").update({ engagement_metrics: metrics }).eq("id", post.id);
          updated++;
        }
      }
...
async function publishPost(supabase: any, post: any) {
  ...
  return { success: !allFailed, post_id: post.id, platform_post_ids: platformPostIds, errors };
}

async function publishToMeta(connections: any[], post: any, platform: string) {
  const fbConn = connections?.find((c: any) => c.platform === "facebook_page" && c.is_active);
  const fbUserConn = connections?.find((c: any) => c.platform === "facebook" && c.is_active);

  if (!fbConn && !fbUserConn) throw new Error("No Facebook connection found");
  ...
  if (platform === "instagram") {
    ...
    const igResp = await fetch(`https://graph.facebook.com/v21.0/${pageId}?fields=instagram_business_account&access_token=${token}`);
    const igData = await igResp.json();
    const igAccountId = igData.instagram_business_account?.id;
    if (!igAccountId) throw new Error("No Instagram Business Account linked to this page");

    if (post.media_urls?.length > 0) {
      const containerBody: any = { caption: post.content, access_token: token };
      ...
      const containerResp = await fetch(`https://graph.facebook.com/v21.0/${igAccountId}/media`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(containerBody),
      });
      const containerData = await containerResp.json();
      if (containerData.error) throw new Error(containerData.error.message);

      const publishResp = await fetch(`https://graph.facebook.com/v21.0/${igAccountId}/media_publish`, {
...
async function publishToLinkedIn(connections: any[], post: any) {
  const linkedinConn = connections?.find((c: any) => c.platform === "linkedin" && c.is_active);
  ...
  if (platformPostIds.facebook) {
    try {
      ...
      if (fbConn) {
        ...
        const data = await resp.json();
        if (!data.error) {
          metrics.facebook = { likes: data.likes?.summary?.total_count || 0, comments: data.comments?.summary?.total_count || 0, shares: data.shares?.count || 0 };
        }
      }
    } catch (e) { console.error("FB engagement fetch error:", e); }
  }
  ...
  if (platformPostIds.instagram) {
    try {
      ...
      if (fbConn) {
        ...
        const data = await resp.json();
        if (!data.error) {
          metrics.instagram = { likes: data.like_count || 0, comments: data.comments_count || 0, impressions: data.impressions || 0, reach: data.reach || 0 };
        }
      }
    } catch (e) { console.error("IG engagement fetch error:", e); }
  }
  ...
  if (platformPostIds.linkedin) {
    try {
      ...
      if (liConn) {
        ...
        const data = await resp.json();
        metrics.linkedin = { likes: data.likes?.count || 0, comments: data.comments?.count || 0 };
      }
    } catch (e) { console.error("LinkedIn engagement fetch error:", e); }
  }
