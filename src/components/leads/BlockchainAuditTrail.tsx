...
export function BlockchainAuditTrail({ leadId }: BlockchainAuditTrailProps) {
  ...
  const handleExportPDF = () => {
    ...
    doc.roundedRect(margin, y, pageW - margin * 2, 52, 2, 2, 'F');
    doc.setDrawColor(180);
    doc.roundedRect(margin, y, pageW - margin * 2, 52, 2, 2, 'S');

    y += 7;
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('DIGITAL ATTESTATION', margin + 4, y);
    y += 6;

    doc.setFontSize(8);
    doc.setFont('helvetica', 'normal');
    const attestLines = [
      'I hereby certify that the data contained in this report has been generated by the LeadThru platform\'s',
      'cryptographic audit system. Each block in the chain is sealed with a SHA-256 hash linked to its predecessor,',
      'forming a tamper-evident ledger. Any modification to any record in this chain would invalidate all subsequent',
      'hashes, making unauthorized alterations immediately detectable.',
    ];
    attestLines.forEach(line => {
    ...
    const docFingerprint = blocks.length > 0 ? blocks[blocks.length - 1].sha256_hash.slice(0, 16) : 'N/A';

    const sigDetails = [
      ['Signed By:', 'LeadThru Automated Integrity System'],
      ['Timestamp:', `${dateStr} at ${timeStr} (UTC${now.getTimezoneOffset() > 0 ? '-' : '+'}${String(Math.abs(now.getTimezoneOffset() / 60)).padStart(2, '0')}:${String(Math.abs(now.getTimezoneOffset() % 60)).padStart(2, '0')})`],
      ['Document Hash:', docFingerprint + '...'],
      ['Verification:', 'https://leadthru.com/verify/' + leadId.slice(0, 8)],
    ];

    sigDetails.forEach(([label, value]) => {
      doc.setFont('helvetica', 'bold');
